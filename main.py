...
os.environ['KIVY_GL_BACKEND'] = 'sdl2'  # Force SDL2 backend
...
class CameraWidget(Image):
    ...
    def _update_texture(self, dt):
        """Updates the image texture on the main thread"""
        try:
            ...
            if results.multi_face_landmarks:
                ...
                if blinking_ratio > self.BLINK_THRESHOLD:
                    ...
                    # Handle blink selection (will need to be connected to keyboard)
                    if self.blinking_frames == self.frames_to_blink:
                        # Only dispatch blink if enough frames have passed
                        current_time = time.time()
                        if current_time - self.last_blink_time >= self.blink_cooldown:  # Add minimum blink delay
            ...
            print(f"Error updating texture: {str(e)}")
    ...
    def _update_loop(self):
        while not self._stop_update:
            try:
                ...
                print(f"Error in camera update loop: {str(e)}")
                break
...
class KeyboardWidget(Image):
    ...
    def update(self, dt):
        ...
        # Convert to texture
        buf = cv2.flip(self.keyboard, 0).tobytes()  # Changed from tostring()
        texture = Texture.create(size=(self.keyboard.shape[1], self.keyboard.shape[0]), colorfmt='bgr')
        texture.blit_buffer(buf, colorfmt='bgr', bufferfmt='ubyte')
        self.texture = texture
...
class BoardWidget(Image):
    ...
    def update(self, dt):
        ...
        # Convert to texture
        buf = cv2.flip(self.board, 0).tobytes()  # Changed from tostring()
        texture = Texture.create(size=(self.board.shape[1], self.board.shape[0]), colorfmt='bgr')
        texture.blit_buffer(buf, colorfmt='bgr', bufferfmt='ubyte')
        self.texture = texture
